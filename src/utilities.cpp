#include "ekf_robot_control/utilities.hpp"
...
void Utilities::getOdometryMeasurement(const nav_msgs::msg::Odometry &msg, ExtendedKalmanFilter &ekf_, Eigen::VectorXd &measurement, Eigen::VectorXd &measurementCovariance, double twoDimensionalMode){
...
void Utilities::getAccelerationMeasurement(const sensor_msgs::msg::Imu &msg, ExtendedKalmanFilter &ekf_, Eigen::VectorXd &measurement, Eigen::VectorXd &measurementCovariance, double twoDimensionalMode){
    ...
    // If IMU Provides Orientation
        ...
        if (fabs(Attitude.length() - 1.0) > 0.01) {
            RCLCPP_WARN_ONCE(rclcpp::get_logger("rclcpp"), "An input was not normalized, this should NOT happen, but will normalize.");
            Attitude.normalize();
        }
        idle.setRotation(Attitude);
    }

    // Rotate the gravitational acceleration in idle state and subtract it from actual IMU accelerations
